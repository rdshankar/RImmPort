...
1) MySQL formatted study data: 
       ...
       Unzips the files. 
       Loads private database instance. 
...
2) Tab: 
       ...
       Passes the folder where the zip files are located to an RImmPort function that builds SQLite db. 
       Connects to the database. 
       Sets the ImmPort data source to the connection handle. 
       Invokes RImmPort functions.

User downloads study data of interest from the ImmPort website (http://www.immport.org) **. Depending on the format, loads the data into a local MySQL database. The user installs the RImmPort package, loads the RImmPort library, connects to the local ImmPort database, and calls RImmPort methods to load sudy data from the database into R. Please refer to the RImmPort_Article.pdf for a detailed discussion on RImmPort.
