...
User downloads study data of interest from the ImmPort website ( http://www.immport.org ) **. Depending on the file format MYSQL Or Tab it loads the data into a local MySQL or SQLlite database repectively. The user installs the RImmPort package, loads the RImmPort library, connects to the local ImmPort database, and calls RImmPort methods to load study data from the database into R. Please refer to the RImmPort_Article.pdf for a detailed discussion on RImmPort.
...
### Load the data in these zip files into a local MySQL database
